...
class StableDiffusionXLAdapterImg2ImgPipeline(
    ...
    def __call__(
        ...
        input_image: Union[torch.Tensor, PIL.Image.Image, List[PIL.Image.Image]] = None,
        ...
        input_image = self.image_processor.preprocess(input_image, height=height, width=width).to(dtype=torch.float32)
        ...
        if not output_type == "latent":
            ...
            adapter_image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]

            # cast back to fp16 if needed
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
        else:
            adapter_image = latents
            return StableDiffusionXLPipelineOutput(images=adapter_image)

        adapter_image = self.image_processor.postprocess(adapter_image, output_type=output_type)
