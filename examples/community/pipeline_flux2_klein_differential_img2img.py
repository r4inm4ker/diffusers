...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from pipeline_flux2_klein_differential_img2img import Flux2KleinDifferentialImg2ImgPipeline
        ...
        >>> # Grayscale change map: bright = change more, dark = keep original
        >>> change_map = load_image("https://example.com/map.jpg")

        >>> result = pipe(
        ...     prompt=prompt,
        ...     image=image,
        ...     mask_image=change_map,
        ...     strength=1.0,
        ...     num_inference_steps=50,
        ... ).images[0]
        >>> result.save("flux2klein_differential.png")
        ```
"""
...
class Flux2KleinDifferentialImg2ImgPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    Flux2 Klein pipeline for differential diffusion image-to-image with optional reference image conditioning.

    Differential diffusion uses a continuous grayscale change map instead of a binary mask. Regions where the map
    is bright (closer to 1) are more free to deviate from the source image, while dark regions (closer to 0) are
    preserved. This provides fine-grained, per-pixel control over the strength of the transformation.

    Reference:
    - Flux2 Klein: [https://bfl.ai/blog/flux2-klein-towards-interactive-visual-intelligence](https://bfl.ai/blog/flux2-klein-towards-interactive-visual-intelligence)
    - Differential Diffusion: [https://differential-diffusion.github.io/](https://differential-diffusion.github.io/)
    ...
    def check_inputs(
        ...
        if image is None:
            raise ValueError("`image` has to be provided for differential diffusion.")

        if mask_image is None:
            raise ValueError("`mask_image` (change map) has to be provided for differential diffusion.")
    ...
    def __call__(
        ...
        r"""
        Function invoked when calling the pipeline for differential diffusion image-to-image generation.
        ...
        Args:
            ...
            image_reference (`torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.Tensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`, *optional*):
                `Image`, numpy array or tensor representing an image batch to be used as a reference for conditioning
                the generated region. For both numpy array and pytorch tensor, the expected value range is between
                `[0, 1]`. If it's a tensor or a list of tensors, the expected shape should be `(B, C, H, W)` or
                `(C, H, W)`. If it is a numpy array or a list of arrays, the expected shape should be
                `(B, H, W, C)` or `(H, W, C)`. It can also accept image latents directly, in which case encoding is
                skipped.
            mask_image (`torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.Tensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                Grayscale change map controlling how much each pixel region can change. Values close to 1 (white)
                allow maximum change, values close to 0 (black) preserve the original image. This is different from
                binary inpainting masks — intermediate values provide smooth, per-pixel control over the strength
                of transformation. If `mask_image` is a PIL image, it is converted to a single channel (luminance)
                before use.
        ...
        # Differential diffusion: pre-compute per-step threshold masks
        # At each step i, pixels with mask value > i/N are allowed to change
            ...
            for i, t in enumerate(timesteps):
                ...
                if i < len(timesteps) - 1:
                    ...
                    image_latent = self.scheduler.scale_noise(
                        image_latent, torch.tensor([noise_timestep]), noise
                    )

                    # Apply per-step differential mask
